...
class CovAdvectionDiffusion2D:
    ...
    def __init__(self,grid,mod0,par=None,bc = 3, ww = None) -> None:
        self.grid = grid
        self.type = "cov-advection-diffusion-2D-bc%d"%(bc)
        self.Q = None
        self.Q_fac = None
        self.data = None
        self.r = None
        ...
        self.fitQ0 = True
        if par is None:
            par = np.array([-1,-1,0.1,0.1,0.1,0,np.log(100)],dtype="float64")
        self.setPars(par)
    ...
    def initFit(self,data,**kwargs):
        ...
        if self.fitQ0:
            ...
            par = np.hstack([-1,-1,0.1,0.1,0.1,0,x0,np.log(100)],dtype="float64")
        else:
            par = np.array([-1,-1,0.1,0.1,0.1,0,np.log(100)],dtype="float64")
    ...
    def makeQ(self, par, grad = True):
        ...
        if grad:
            ...
            dQ.append(1/(dt*sigma)*tdQ.tocsc())
            # vx
            dvv = np.array([1,0],dtype = "float64")
            ...
            dQ.append(1/(dt*sigma)*tdQ.tocsc())
            # vy
            dvv = np.array([0,1],dtype = "float64")
            dHs = dvv[:,np.newaxis]*vv[np.newaxis,:] + vv[:,np.newaxis]*dvv[np.newaxis,:]
            dA = - self.Ah(dHs)*dt
            tdQ = sparse.bmat([[sparse.csc_matrix((Ns,Ns)), - Qs@iDv@dA,sparse.csc_matrix((Ns,(T-2)*Ns))]])
            for t in range(T-2):
