...
class CovAdvectionVarIDiffusion2D:
    ...
    def __init__(self,grid,mod0,par=None,bc = 3, ww = None) -> None:
        ...
        self.fitQ0 = True
        if par is None:
            par = np.hstack([[-1]*9,[-1]*9,0.1,0,np.log(100)],dtype = "float64")
        self.setPars(par)
    ...
    def getPars(self,onlySelf = True) -> np.ndarray:
        if onlySelf:
            ...
            return(np.hstack([self.kappa,self.gamma,self.lamb,self.sigma,self.mod0.getPars()[:-1],self.tau],dtype = "float64"))
    ...
    def initFit(self,data,**kwargs):
        ...
        if self.fitQ0:
            ...
            par = np.hstack([[-1]*9,[-1]*9,0.1,0,x0,np.log(100)],dtype = "float64")
        else:
            par = np.hstack([[-1]*9,[-1]*9,0.1,0,np.log(100)],dtype = "float64")
    ...
    def print(self,par):
        if par.size >= 21:
            s1 ="| \u03BA = %2.2f"%(np.exp(par[0:9]).mean()) +  ", \u03B3 = %2.2f"%(np.exp(par[9:18]).mean()) +  ", \u039B = %2.2f"%(np.exp(par[18])) +", \u03C3 = %2.2f"%(np.exp(par[19])) +  ", \u03C4 = %2.2f\n"%(np.exp(par[-1])) 
            s1 += " Q0: " + self.mod0.print(par[20:])[:-10]
            return s1
        else:
            return("| \u03BA = %2.2f"%(np.exp(par[0:9]).mean()) +  ", \u03B3 = %2.2f"%(np.exp(par[9:18]).mean()) +  ", \u039B = %2.2f"%(np.exp(par[18])) +", \u03C3 = %2.2f"%(np.exp(par[19])) +  ", \u03C4 = %2.2f"%(np.exp(par[20])))
    ...
    def makeQ(self, par, grad = True):
        ...
        if par.size > 21:
           Q0,_,dQ0 = self.mod0.makeQ(par[20:],grad = grad) 
        else:
