...
class SeperableSpatialTemporalIDiffusion2D:
    ...
    def __init__(self,grid,par=None,bc = 3) -> None:
        self.grid = grid
        self.type = "seperable-spatial-temporal-2D-bc%d"%(bc)
        self.Q = None
        self.Q_fac = None
        self.data = None
        self.r = None
        ...
        self.AHnew = None
        if par is None: 
            par = np.hstack([[-1]*9,[-1]*9,0.1,1,np.log(100)],dtype="float64")
            self.setPars(par)
        else:
            self.setQ(par = par)
    ...
    def print(self,par):
        return("| \u03BA = %2.2f"%(np.exp(par[0:9]).mean()) +  ", \u03B3 = %2.2f"%(np.exp(par[9:18]).mean()) + ", a = %2.2f"%(par[18]) + ", \u03C3	 = %2.2f"%(np.exp(par[19])) +  ", \u03C4 = %2.2f"%(np.exp(par[20])))
