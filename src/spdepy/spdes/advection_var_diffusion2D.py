...
class AdvectionVarDiffusion2D:
    def __init__(self, grid, mod0, par=None, bc = 3) -> None:
        self.grid = grid
        self.type = "advection-var-diffusion-2D-bc%d"%(bc)
        self.Q = None
        self.Q_fac = None
        self.data = None
        self.r = None
        ...
        self.fitQ0 = True
        if par is None:
            par = np.hstack([[-1]*9,[-1]*9,[0.1]*9,[0.1]*9,0.1,0.1,0,np.log(100)],dtype = "float64")
            self.setPars(par)
        else:
            self.setQ(par = par)
    ...
    def initFit(self,data, **kwargs):
        ...
        if self.fitQ0:
            ...
            par = np.hstack([[-1]*9,[-1]*9,[0.1]*9,[0.1]*9,0.1,0.1,0,x0,np.log(100)],dtype = "float64")
        else:
            par = np.hstack([[-1]*9,[-1]*9,[0.1]*9,[0.1]*9,0.1,0.1,0,np.log(100)],dtype = "float64")
        self.data = data
        if self.data.ndim == 2:
    ...
    def print(self,par):
        if par.size > 40:
            s1 = "| \u03BA = %2.2f"%(np.exp(par[0:9]).mean()) +  ", \u03B3 = %2.2f"%(np.exp(par[9:18]).mean()) + ", vx = %2.2f"%((par[18:27]).mean()) + ", vy = %2.2f"%((par[27:36]).mean()) + ", wx = %2.2f"%(par[36]) + ", wy = %2.2f"%(par[37]) + ", \u03C3 = %2.2f"%(np.exp(par[38])) +  ", \u03C4 = %2.2f\n"%(np.exp(par[-1]))
            s1 = s1 + " Q0: " + self.mod0.print(par[39:])[:-10]
            return s1
        else:
            return "| \u03BA = %2.2f"%(np.exp(par[0:9]).mean()) +  ", \u03B3 = %2.2f"%(np.exp(par[9:18]).mean()) + ", vx = %2.2f"%((par[18:27]).mean()) + ", vy = %2.2f"%((par[27:36]).mean()) + ", wx = %2.2f"%(par[36]) + ", wy = %2.2f"%(par[37]) + ", \u03C3 = %2.2f"%(np.exp(par[38])) +  ", \u03C4 = %2.2f"%(np.exp(par[-1]))
    ...
    def makeQ(self, par, grad = True):
        ...
        if par.size > 40:
            Q0, _ ,dQ0 = self.mod0.makeQ(par = par[39:],grad = grad)
        else:
