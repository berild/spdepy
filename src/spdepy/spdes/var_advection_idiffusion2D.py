...
class VarAdvectionIDiffusion2D:
    def __init__(self, grid, mod0, par = None, bc = 3) -> None:
        self.grid = grid
        self.type = "var-advection-idiffusion-2D-bc%d"%(bc)
        self.Q = None
        self.Q_fac = None
        self.data = None
        self.r = None
        ...
        self.fitQ0 = True
        if par is None:
            par = np.hstack([-1,-1,[0.01]*18,0,np.log(100)],dtype = "float64")
            self.setPars(par)
        else:
            self.setQ(par = par)
    ...
    def initFit(self,data, **kwargs):
        ...
        if self.fitQ0:
            ...
            par = np.hstack([-1,-1,[0.01]*18,0,x0,np.log(100)],dtype = "float64")
        else:
            par = np.hstack([-1,-1,[0.01]*18,0,np.log(100)],dtype = "float64")
        self.data = data
        if self.data.ndim == 2:
    ...
    def print(self,par):
        if par.size > 22:
            s1 ="| \u03BA = %2.2f"%(np.exp(par[0])) +  ", \u03B3 = %2.2f"%(np.exp(par[1])) + ", wx = %2.2f"%(par[2:11].mean()) + ", wy = %2.2f"%(par[11:20].mean()) +", \u03C3 = %2.2f"%(np.exp(par[20])) +  ", \u03C4 = %2.2f\n"%(np.exp(par[-1])) 
            s1 += " Q0: " + self.mod0.print(par[21:])[:-10]
        else:
            return("| \u03BA = %2.2f"%(np.exp(par[0])) +  ", \u03B3 = %2.2f"%(np.exp(par[1])) + ", wx = %2.2f"%(par[2:11].mean()) + ", wy = %2.2f"%(par[11:20].mean()) +", \u03C3 = %2.2f"%(np.exp(par[20])) +  ", \u03C4 = %2.2f"%(np.exp(par[21])))
    ...
    def makeQ(self, par, grad = True):
        ...
        kappa = np.exp(par[0])
        gamma = np.exp(par[1])
        ws = self.grid.evalAdv(par[2:20])
        sigma = np.exp(par[20])
        # components
        Hs = gamma*np.eye(2)
        Dk =  kappa*sparse.eye(Ns).tocsc()
        A_H = self.Ah(Hs)
        As = Dv@Dk
        Qs = As.transpose()@iDv@As
        A = Dv + (Dv@Dk - A_H + self.Aw(ws))*dt
        if par.size > 22:
            Q0, _, dQ0 = self.mod0.makeQ(par[21:], grad = grad)
        else:
