...
class CovAdvectionHaDiffusion2D:
    ...
    def __init__(self,grid,mod0,par=None,bc = 3, ww = None) -> None:
        self.grid = grid
        self.type = "cov-advection-ha-diffusion-2D-bc%d"%(bc)
        self.Q = None
        self.Q_fac = None
        self.data = None
        self.r = None
        ...
        self.fitQ0 = True
        if par is None:
            par = np.array([-1,-1,0.01,0.01,0.01,0,np.log(100)],dtype="float64")
        self.setPars(par)
    ...
    def transDiff(self,par = None):
        ...
        self.tvx = np.sqrt(np.exp(par[1])*sinh_aV/aV*(par[2]+ aV))
        self.tvy = np.sqrt(np.exp(par[1])*sinh_aV/aV*(-par[2]+ aV))
    ...
    def initFit(self,data,**kwargs):
        ...
        if self.fitQ0:
            ...
            par = np.hstack([-1,-1,0.01,0.01,0.01,0,x0,np.log(100)],dtype="float64")
        else:
            par = np.array([-1,-1,0.01,0.01,0.01,0,np.log(100)],dtype="float64")
    ...
    def print(self,par):
        if par.size > 7:
            s1 = "| \u03BA = %2.2f"%(np.exp(par[0])) +  ", \u03B3 = %2.2f"%(np.exp(par[1])) + ", vx = %2.2f"%(par[2])+ ", vy = %2.2f"%(par[3])+ ", \u039B = %2.2f"%(par[4]) +", \u03C3 = %2.2f"%(np.exp(par[5])) +  ", \u03C4 = %2.2f\n"%(np.exp(par[-1]))
            s1 += " Q0: " + self.mod0.print(par[6:])[:-10]
            return s1
        else:
            return("| \u03BA = %2.2f"%(np.exp(par[0])) +  ", \u03B3 = %2.2f"%(np.exp(par[1])) + ", vx = %2.2f"%(par[2])+ ", vy = %2.2f"%(par[3])+ ", \u039B = %2.2f"%(par[4]) +", \u03C3 = %2.2f"%(np.exp(par[5])) +  ", \u03C4 = %2.2f"%(np.exp(par[6])))
    ...
    def makeQ(self, par, grad = True):
        ...
        if par.size > 7:
            Q0, _, dQ0 = self.mod0.makeQ(par[6:],grad = grad)
        else:
