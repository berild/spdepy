...
class VarAdvectionHaDiffusion2D:
    def __init__(self, grid,mod0, par=None, bc = 3) -> None:
        self.grid = grid
        self.type = "var-advection-ha-diffusion-2D-bc%d"%(bc)
        self.Q = None
        self.Q_fac = None
        self.data = None
        self.r = None
        ...
        self.fitQ0 = True
        if par is None:
            par = np.hstack([-1,-1,0.1,0.1,[0.1]*18,0,np.log(100)],dtype="float64")
            self.setPars(par)
        else:
            self.setQ(par = par)
    ...
    def initFit(self,data, **kwargs):
        ...
        if self.fitQ0:
            ...
            par = np.hstack([-1,-1,0.1,0.1,[0.1]*18,0,x0,np.log(100)],dtype="float64")
        else:
            par = np.hstack([-1,-1,0.1,0.1,[0.1]*18,0,np.log(100)],dtype="float64")
        self.data = data
        if self.data.ndim == 2:
    ...
    def print(self,par):
        if par.size > 24:
            s1 = "| \u03BA = %2.2f"%(np.exp(par[0])) +  ", \u03B3 = %2.2f"%(np.exp(par[1]))+  ", vx = %2.2f"%(par[2]) + ", vy = %2.2f"%(par[3]) + ", wx = %2.2f"%(par[4:13].mean()) + ", wy = %2.2f"%(par[13:22].mean()) +", \u03C3 = %2.2f"%(np.exp(par[22])) +  ", \u03C4 = %2.2f\n"%(np.exp(par[-1])) 
            s1 = s1 + " Q0: " + self.mod0.print(par[23:])[:-10]
            return s1
        else:
    ...
    def makeQ(self, par, grad = True):
        ...
        if par.size > 24:
            Q0, _, dQ0 = self.mod0.makeQ(par[23:],grad = grad)
        else:
