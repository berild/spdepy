...
class AdvectionIDiffusion2D:
    ...
    def __init__(self,grid,mod0, par=None, bc = 3) -> None:
        ...
        self.fitQ0 = True
        if par is None:
            par = np.array([-1,-1,0.1,0.1,0,np.log(100)],dtype="float64")
            self.setPars(par)
        else:
            self.setQ(par = par)
    ...
    def initFit(self,data,**kwargs):
        ...
        if self.fitQ0:
            ...
            par = np.hstack([-1,-1,0.1,0.1,0,x0,np.log(100)],dtype="float64")
        else:
            par = np.array([-1,-1,0.1,0.1,0,np.log(100)],dtype="float64")
        self.data = data
        if self.data.ndim == 2:
    ...
    def print(self,par):
        if par.size > 6:
            s1 = ("| \u03BA = %2.2f"%(np.exp(par[0])) +  ", \u03B3 = %2.2f"%(np.exp(par[1])) + ", wx = %2.2f"%(par[2]) + ", wy = %2.2f"%(par[3]) +", \u03C3 = %2.2f"%(np.exp(par[4])) +  ", \u03C4 = %2.2f\n"%(np.exp(par[-1])))
            s1 = s1 + " Q0: " + self.mod0.print(par[5:])[:-10]
            return s1
        else:
            return("| \u03BA = %2.2f"%(np.exp(par[0])) +  ", \u03B3 = %2.2f"%(np.exp(par[1])) + ", wx = %2.2f"%(par[2]) + ", wy = %2.2f"%(par[3]) +", \u03C3 = %2.2f"%(np.exp(par[4])) +  ", \u03C4 = %2.2f"%(np.exp(par[-1])))
    ...
    def makeQ(self, par, grad = True):
        ...
        if par.size > 6:
            Q0, _, dQ0 = self.mod0.makeQ(par = par[5:], grad = grad)
        else:
            if self.mod0.Q is None:
                self.mod0.setQ(par = par[5:])
            Q0 = self.mod0.Q
        # precision matrix Q
