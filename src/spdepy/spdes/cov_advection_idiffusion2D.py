...
class CovAdvectionIDiffusion2D:
    ...
    def __init__(self,grid,mod0, par=None,bc = 3, ww = None) -> None:
        ...
        self.fitQ0 = True
        if par is None:
            par = np.array([-1,-1,0.1,0,np.log(100)],dtype = "float64")
        self.setPars(par)
    ...
    def initFit(self,data,**kwargs):
        ...
        if self.fitQ0:
            ...
            par = np.hstack([-1,-1,0.1,0,x0,np.log(100)],dtype = "float64")
        else:
            par = np.array([-1,-1,0.1,0,np.log(100)],dtype = "float64")
    ...
    def print(self,par):
        if par.size > 5:
            s1 ="| \u03BA = %2.2f"%(np.exp(par[0])) +  ", \u03B3 = %2.2f"%(np.exp(par[1])) + ", \u039B = %2.2f"%(par[2]) +", \u03C3 = %2.2f"%(np.exp(par[3])) +  ", \u03C4 = %2.2f\n"%(np.exp(par[-1])) 
            s1 += " Q0: " + self.mod0.print(par[4:])[:-10]
            return(s1)
        else:
            return("| \u03BA = %2.2f"%(np.exp(par[0])) +  ", \u03B3 = %2.2f"%(np.exp(par[1])) + ", \u039B = %2.2f"%(par[2]) +", \u03C3 = %2.2f"%(np.exp(par[3])) +  ", \u03C4 = %2.2f"%(np.exp(par[4])))
    ...
    def makeQ(self, par, grad = True):
        ...
        if par.size > 5:
            Q0, _, dQ0 = self.mod0.makeQ(par[4:],grad = grad)
        else:
