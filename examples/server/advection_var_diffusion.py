...
def fit(bc):
    ...
    mod = sp.model(grid = sp.grid(x=data['x'], y=data['y'], t = data['t'],extend = 5),
         spde = 'advection-var-diffusion',  ha = True, bc = bc, anisotropic = True,mod0 = mod0)
    
    mod.fit(data = data['mut'],verbose = True,lr = lr,
            end = "../fits/advection_var_diffusion_ha_bc%d"%bc)

    x0 = mod.getPars()
    
    mod.fit(data = data['mut'],verbose = True,lr = lr2,
            end = "../fits/advection_var_diffusion_ha_bc%d"%bc, x0 = x0)
    ...
    mod.fit(data = data['mut'],verbose = True,lr = lr,
            end = "../fits/advection_var_diffusion_ani_bc%d"%bc)
    
    x0 = mod.getPars()
    
    mod.fit(data = data['mut'],verbose = True,lr = lr2,
            end = "../fits/advection_var_diffusion_ani_bc%d"%bc, x0 = x0)
    ...
    mod = sp.model(grid = sp.grid(x=data['x'], y=data['y'], t = data['t'],extend = 5),
         spde = 'advection-var-diffusion', ha = False, bc = bc, anisotropic = False,mod0 = mod0)
    
    mod.fit(data = data['mut'],verbose = True,lr = lr,
            end = "../fits/advection_var_diffusion_bc%d"%bc)
    
    x0 = mod.getPars()
    
    mod.fit(data = data['mut'],verbose = True,lr = lr2,
            end = "../fits/advection_var_diffusion_bc%d"%bc, x0 = x0)
