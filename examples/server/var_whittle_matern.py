...
def fit(bc):
    ...
    mod.fit(data = data['muf'],verbose = True,lr = lr,
            end = "../fits/var_whittle_matern_ha_bc%d"%bc)
    x0 = mod.getPars()            
    mod.fit(data = data['muf'],verbose = True,lr = lr2, 
            end = "../fits/var_whittle_matern_ha_bc%d"%bc, x0 = x0)
    
    ### ANI
    mod = sp.model(grid = sp.grid(x=data['x'], y=data['y'],extend = 5),
         spde = 'var-whittle-matern', ha = False, bc = bc, anisotropic = True)
    mod.fit(data = data['muf'],verbose = True,lr = lr, 
            end = "../fits/var_whittle_matern_ani_bc%d"%bc)
    x0 = mod.getPars()            
    mod.fit(data = data['muf'],verbose = True,lr = lr2, 
            end = "../fits/var_whittle_matern_ani_bc%d"%bc, x0 = x0)
    
    ### ISO
    mod = sp.model(grid = sp.grid(x=data['x'], y=data['y'],extend = 5),
         spde = 'var-whittle-matern', ha = False, bc = bc, anisotropic = False)
    mod.fit(data = data['muf'],verbose = True,lr = lr, 
            end = "../fits/var_whittle_matern_bc%d"%bc)
    x0 = mod.getPars()            
    mod.fit(data = data['muf'],verbose = True,lr = lr2, 
            end = "../fits/var_whittle_matern_bc%d"%bc, x0 = x0)
