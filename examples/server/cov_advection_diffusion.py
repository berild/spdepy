...
def fit(bc):
    ...
    mod = sp.model(grid = sp.grid(x=data['x'], y=data['y'], t = data['t'],extend = 5),
         spde = 'cov-advection-diffusion',  ha = True, bc = bc, anisotropic = True, mod0 = mod0)
    
    ww = mod.grid.assimilate_adv(data['we'],data['wn'])
    
    mod.fit(data = data['mut'],verbose = True,lr = lr,
            end = "../fits/cov_advection_diffusion_ha_bc%d"%bc, ww = ww)

    x0 = mod.getPars()
    
    mod.fit(data = data['mut'],verbose = True,lr = lr2,
            end = "../fits/cov_advection_diffusion_ha_bc%d"%bc, x0 = x0)
    ...
    mod = sp.model(grid = sp.grid(x=data['x'], y=data['y'], t = data['t'],extend = 5),
         spde = 'cov-advection-diffusion', ha = False, bc = bc, anisotropic = True, mod0 = mod0)
    
    mod.fit(data = data['mut'],verbose = True,lr = lr,
            end = "../fits/cov_advection_diffusion_ani_bc%d"%bc, ww = ww)

    x0 = mod.getPars()
    
    mod.fit(data = data['mut'],verbose = True,lr = lr2,
            end = "../fits/cov_advection_diffusion_ani_bc%d"%bc, x0 = x0)
    ...
    mod = sp.model(grid = sp.grid(x=data['x'], y=data['y'], t = data['t'],extend = 5),
         spde = 'cov-advection-diffusion', ha = False, bc = bc, anisotropic = False, mod0 = mod0)
    
    mod.fit(data = data['mut'],verbose = True,lr = lr,
            end = "../fits/cov_advection_diffusion_bc%d"%bc, ww = ww)
    
    x0 = mod.getPars()
    
    mod.fit(data = data['mut'],verbose = True,lr = lr2,
            end = "../fits/cov_advection_diffusion_bc%d"%bc, x0 = x0)
